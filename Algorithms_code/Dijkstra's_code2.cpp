...
/*for printing shortest path source to distination*/
int parent[1000];
        ...
        */
        for(i = 0; i < G[u].size(); i++) {
            ...
            */
            if(d[v] > d[u] + w) {
                d[v] = d[u] + w;
                parent[v]=u;     /*save parent of child for finding total shortest path..IF not required we can ignore this */
                Q.push(pii(d[v], v));
            }
        }
    }
    ...
    */
    while(scanf("%d %d", &n, &e) == 2) {
        ...
        */
        ...
        printf("Shortest path from source %d to %d distintion -> %d ",1,n, start);
        printPath(n);
